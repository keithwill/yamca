...
/// settings UI. Both tiers live on disk: the global tier (which carries API keys and isn't
/// repo-scoped) under the OS per-user config directory via
/// <see cref="GlobalSettingsStore"/>, and the project tier under the repository's
/// <c>.yamca</c> directory via <see cref="ProjectSettingsStore"/>.</summary>
public sealed class SettingsLocation
{
    /// <summary>Absolute path of the global-tier settings file, shown in the UI so users
    /// know where their endpoints/API keys are stored.</summary>
    public string GlobalFile => Path.Combine(GlobalSettingsStore.ResolveDefaultDirectory(), "global.json");
